...
Gradient Descent라는 것은 어떠한 함수가 최소값을 가지게하는 point를 찾아가는 일종의 수학 테크닉이다.이 gradient descent를 통해서 cost function이 최소가 되게하는$$(\theta_{0},\theta_{1})$$을 구할 수 있다.
...
<span style="font-size: 12pt;"><strong>1) 1차함수 형태의 hypothesis function을 세우고 무작위로 $$\theta_{0}$$과 $$\theta_{1}$$을 set한다.</strong></span>

<span style="font-size: 12pt;"><strong>2) learning rate alpha를 정하고 위의 update식에 따라 model parameter($$\theta_{0}$$ $$\theta_{1}$$)을 반복 학습시킨다. 이를 gradient descent(경사하강법)이라고 한다.</strong></span>

<span style="font-size: 12pt;"><strong>3) $$J(\theta)$$가 수렴했으면 지금의 $$\theta_{0}$$와 $$\theta_{1}$$을</strong> 이<strong> model parameter로 사용한다</strong></span>