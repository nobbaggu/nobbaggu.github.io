...
![image](https://nobbaggu.github.io/images/2018/09/no-name-1.png){: width="50%" height="50%"}
...
![image](https://nobbaggu.github.io/images/2018/09/no-name-3.png){: width="50%" height="50%"}

위는 mean(평균)이 $$\mu$$이고 variance(분산)이 $$\sigma^{2}$$인 가우스 분포이다.

어떤 데이터의 집합이 가우스 분포를 따른다고 할 때 다음과 같이 나타낸다.   $$x^{(i)}\sim N(\mu,\sigma^{2})$$
...
training dataset $$x^{(1)},x^{(2)},\cdot\cdot\cdot,x^{(m)}$$이 있다고 해보자.

먼저 anomaly 에 대해서는 p가 작고 normal data에 대해서는 p를 크게 해 줄 것 같은 적절한 feature를 선정한다. feature의 수는 n이라고 하고$$(x^{(i)}\in \mathbb{R}^{n})$$  data들이 가우스 분포를 따른다고 하면 model 식은 다음과 같다.

$$\\p(x;\mu,\sigma^{2})=\frac{1}{\sigma\sqrt{2\pi}}e^{-\frac{(x-\mu)^{2}}{2\sigma^{2}}}$$                      $$\\ \mu=\frac{1}{m}\sum_{i=1}^{m}x^{(i)} \\ \sigma^{2}=\frac{1}{m}\sum_{i=1}^{m}(x^{(i)}-\mu)^{2}$$

각 data의 feature은 각각이 서로 독립적이라고 가정하면 모델은 다음과 같이 표현 가능하다.

$$p(x;\mu,\sigma^{2})=p(x_{1};\mu_{1},\sigma_{1^{2}})p(x_{2};\mu_{2},\sigma_{2}^{2})\cdot\cdot\cdotp(x_{n};\mu_{n},\sigma_{n}^{2})=\prod_{j=1}^{n}p(x_{j};\mu_{j},\sigma_{j}^{2})$$ 

$$\\ \mu_{j}=\frac{1}{m}\sum_{i=1}^{m}x^{(i)}_{j} \\ \sigma^{2}_{j}=\frac{1}{m}\sum_{i=1}^{m}(x^{(i)}_{j}-\mu_{j})^{2}$$ 

$$p(x^{(i)})$$ 는$$x^{(i)}$$ 가 normal한 data일 확률이다. 여기에 새로운 data  $$x_{test}$$가 들어왔을 때 $$p(x_{test})$$ 우리가 정한 어떤 수 $$\epsilon$$보다 작으면 anomaly 라고 판단한다.

$$\\ p(x_{test})> \epsilon:normal(y=0) \\ p(x_{test})< \epsilon:anomaly(y=1)$$ 
...
**training data : normal 6,000개$$x^{(1)},x^{(2)},\cdot\cdot\cdot,x^{(m)}$$** 

**cv(cross validation) data : normal 2,000개 / anomaly 10개 $$(x^{(1)},y^{(1)}),(x^{(2)},y^{(2)}),\cdot\cdot\cdot,(x^{(m_{cv})},y^{(m_{cv})})$$**

**test data : normal 2,000개 / anomaly 10개 $$(x^{(1)},y^{(1)}),(x^{(2)},y^{(2)}),\cdot\cdot\cdot,(x^{(m_{test})},y^{(m_{test})})$$**

&nbsp;

training data는 normal한 데이터만 가지고 이를 사용해 모델을 구한다.

그리고 cv dataset을 사용해서$$\epsilon$$ 를 조정한다. model의 성능을 확인하는 지표로 F score를 활용하면 된다.

$$F=\frac{PR}{P+R}\quad (P:precision\quad R:recall)$$ 

ex) 여러 개의 $$\epsilon$$으로 model $$p(x)$$를 학습시켜 각각의 model의 F score를 비교하여 $$\epsilon$$ 결정
...
우리의 최종 목표는 normal한 data에 대해서는 $$p(x)$$가 매우 크고 anomaly 에 대해서는 $$p(x)$$가 매우 작게 하는 모델을 만드는 것이다.